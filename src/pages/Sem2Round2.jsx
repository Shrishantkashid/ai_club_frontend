...
const Sem2Round2 = ({ onCompleteRound2 }) => {
  ...
  const handleGameWin = (activity, moves, timeTaken) => {
    ...
    // Save the score for this activity
  ...
  const handleSubmit = async () => {
    ...
    // Calculate total score from all completed activities
    const totalMoves = Object.values(activityScores).reduce((sum, score) => sum + (score.moves || 0), 0);
    const totalTime = Object.values(activityScores).reduce((sum, score) => sum + (score.timeTaken || 0), 0);
    
    const score = {
      round: 2,
      moves: totalMoves, // Total moves across all activities
    ...
    try {
      ...
      await fetch(API.SEM2_ROUND2_SUBMIT, {
        method: 'POST',
        headers: {
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          {/* Progress indicator */}
          <div style={{
            backgroundColor: completedActivities.length >= 3 ? 'rgba(74, 222, 128, 0.1)' : 'rgba(251, 191, 36, 0.1)',
            padding: '1rem',
            borderRadius: '8px',
            marginBottom: '1.5rem',
            border: `2px solid ${completedActivities.length >= 3 ? '#4ade80' : '#fbbf24'}`
          }}>
            <p style={{ 
              color: completedActivities.length >= 3 ? '#4ade80' : '#fbbf24',
              fontWeight: 'bold',
              margin: '0 0 0.5rem 0',
              fontSize: '1.1rem'
            }}>
            ...
            {completedActivities.length < 3 ? (
              <p style={{ 
                ...
                ⚠️ You need to complete {3 - completedActivities.length} more activity/activities
              </p>
            ) : (
              <p style={{ 
                color: '#4ade80',
          ...
          {/* Submit button - only enabled when 3+ activities completed */}
          <button
            onClick={handleSubmit}
            disabled={completedActivities.length < 3}
            style={{
              backgroundColor: completedActivities.length >= 3 ? '#4ade80' : '#6b7280',
              color: '#0a192f',
              border: 'none',
              padding: '1rem 2rem',
              fontSize: '1.1rem',
              borderRadius: '30px',
              cursor: completedActivities.length >= 3 ? 'pointer' : 'not-allowed',
              fontWeight: 'bold',
              transition: 'all 0.3s ease',
              opacity: completedActivities.length >= 3 ? 1 : 0.6
            }}
            onMouseEnter={(e) => {
          ...
          >
            {completedActivities.length >= 3 
              ? 'Submit Round 2 ✅' 
              : `Submit Round 2 (${completedActivities.length}/3 Required) ⚠️`}
          </button>
        </div>
      </div>
    )
    ...
    <div style={{
      ...
      <div style={{
          ...
          <div style={{
            ...
            {completedActivities.length < 4 && (
              <div style={{
                ...
                ⚠️ Complete at least 3 activities before submitting Round 2
              </div>
            )}
        ...
        <div style={{ marginTop: '2rem', color: '#94a3b8', fontSize: '0.9rem' }}>
          <p>Complete at least 3 activities to unlock the submit button.</p>
